...
from config import UNDERLYING_ORDER, CONTRACT_MULTIPLIER
from config import REGRESSION_TRAIN_DAYS, REGRESSION_TEST_DAYS
from config import OPTION_RETURN_FILE
...
# ------------------------------------------------------
def calculate_A(info_df, date, training_days=TRAIN_DAYS, tau=TAU):
...
# ------------------------------------------------------
# Baseline Markowitz: contract historical return mean.
# ------------------------------------------------------
def build_contract_return_mean(
    info_df,
    date,
    training_days=24,
    return_path=OPTION_RETURN_FILE,
    return_col="daily_return",
):
    df_today, contract_index = get_daily_contract_index(info_df, date)
    date = pd.to_datetime(date)
    trading_dates = pd.DatetimeIndex(pd.to_datetime(TRADING_DATES)).sort_values()
    history_dates = trading_dates[trading_dates < date][-training_days:]

    if len(history_dates) < training_days:
        raise ValueError("Insufficient historical trading dates.")

    returns = pd.read_csv(return_path)
    required_cols = {"time", "ticker", return_col}
    missing_cols = required_cols - set(returns.columns)
    if missing_cols:
        raise ValueError(f"Missing columns in return table: {sorted(missing_cols)}")

    return_matrix = (
        returns.loc[
            pd.to_datetime(returns["time"]).isin(history_dates),
            ["time", "ticker", return_col],
        ]
        .assign(
            time=lambda x: pd.to_datetime(x["time"]),
            **{return_col: lambda x: pd.to_numeric(x[return_col], errors="raise")},
        )
        .pivot_table(
            index="time",
            columns="ticker",
        ...
        .reindex(index=history_dates, columns=contract_index)
    )

    if return_matrix.isna().any().any():
        missing_contracts = return_matrix.columns[return_matrix.isna().any()].tolist()
        raise ValueError(
            "Missing contract returns in the estimation window: "
            f"{missing_contracts}"
        )

    return return_matrix.mean().to_numpy(dtype=float), contract_index


# ------------------------------------------------------
# Baseline Markowitz: contract historical return variance.
# ------------------------------------------------------
def build_contract_return_variance(
    info_df,
    date,
    training_days=24,
    return_path=OPTION_RETURN_FILE,
    ...
    variance = return_matrix.var(ddof=1).to_numpy(dtype=float)
    return np.diag(variance), contract_index


# ------------------------------------------------------
# Baseline Markowitz: per-contract historical return mean.
# Uses each contract's own latest `training_days` observations before `date`.
# This later definition intentionally overrides the matrix-window version above.
# ------------------------------------------------------
def build_contract_return_mean(
    info_df,
    date,
    ...
    returns = returns.loc[:, ["time", "ticker", return_col]].assign(
        time=lambda x: pd.to_datetime(x["time"]),
        **{return_col: lambda x: pd.to_numeric(x[return_col], errors="raise")},
    )
    returns = returns[
        (returns["time"] < date)
        & (returns["ticker"].isin(contract_index))
    ...
    mean_vector = (
        mean_by_contract
        .reindex(contract_index)
        .fillna(0.0)
        .to_numpy(dtype=float)
    )
    return mean_vector, contract_index


# ------------------------------------------------------
# Baseline Markowitz: per-contract historical return variance.
# Uses each contract's own latest `training_days` observations before `date`.
# This later definition intentionally overrides the matrix-window version above.
# ------------------------------------------------------
def build_contract_return_variance(
    ...
    variance_vector = (
        variance_by_contract
        .reindex(contract_index)
        .where(count_by_contract >= training_days, 100.0)
        .to_numpy(dtype=float)
    )
    return np.diag(variance_vector), contract_index
