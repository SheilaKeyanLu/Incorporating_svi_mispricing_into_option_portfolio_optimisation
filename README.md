...
```
svi_mispricing_signal_construction/   # Stages 1–2 in the pipeline above
...
| 5.4 Residual-Based Mispricing Signal & Persistence | Standardised residuals → daily signal; ACF/PACF, half-life, ANOVA | `svi_mispricing_signal_construction/04_mispricing_signal_construction/`, `halflife_analysis/` |
| Appendix B / 6.2.2 Delta–Gamma Return & Risk Modelling | Option Greeks; portfolio-level `u`, `A = VΣV' + D` | `calc_greeks/`, `model_backtest_comparison/preprocessing.py` |
| 6.2.3 Return-Prediction Models & BL View Construction | Baseline/Model 1–3 regressions, OOS validation, `P`, `Q`, `Ω` construction | `signal_return_prediction/`, `model_backtest_comparison/preprocessing.py` |
...
1. `svi_mispricing_signal_construction/` — run subfolders `01` → `04` in
...
5. `model_backtest_comparison/` — run the four backtest entrypoints
   (`01_bl_long_short_backtest.py` … `04_historical_mean_variance_backtest.py`)
...
Several intermediate/raw files exceed GitHub's 100 MB single-file limit and are
excluded via `.gitignore`:

- Large CSVs under `svi_mispricing_signal_construction/` (raw and intermediate
  option-quote datasets).
- `calc_greeks/option_quotes_with_iv.csv` (~626 MB)
- `calc_greeks/option_quotes_with_greeks.csv` (~801 MB)
- `__pycache__/`, `*.py[cod]`

These files are not committed to version control. To reproduce them, run the
corresponding stage's scripts on the raw data (see Section 4), or obtain the
files from the external data source used in the dissertation (Tonghuashun
iFinD API contract/underlying data plus the proprietary intraday quote
database — see Chapter 4 of the dissertation for details).
...
- [`svi_mispricing_signal_construction/README.md`](svi_mispricing_signal_construction/README.md)
...
(`calc_greeks/` does not currently have its own README — consider adding one
that documents `calc_greeks.py` / `calculate_greeks_for_quotes.py` and notes
the two large excluded CSV outputs listed in Section 5.)
